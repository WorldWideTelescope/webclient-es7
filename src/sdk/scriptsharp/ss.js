...
function _ss() {
  ...
  function parseRegExp(s) {
    if (s[0] == '/') {
      const endSlashIndex = s.lastIndexOf('/');
      if (endSlashIndex > 1) {
        const expression = s.substring(1, endSlashIndex);
        const flags = s.substr(endSlashIndex + 1);
  ...
  function _popStackFrame(e) {
    ...
    const pattern = e.fileName + ':' + e.lineNumber;
    while (isValue(currentFrame) && currentFrame.indexOf(pattern) === -1) {
      stackFrames.shift();
      currentFrame = stackFrames[0];
    }

    const nextFrame = stackFrames[1];
    if (!isValue(nextFrame)) {
      return;
    }

    const nextFrameParts = nextFrame.match(/@(.*):(\d+)$/);
  ...
  function Enumerator(obj, keys) {
    let index = -1;
    const length = keys ? keys.length : obj.length;
    const lookup = keys ? function () {
  ...
  const _formatPlaceHolderRE = /(\{[^\}^\{]+\})/g;
  const _formatters = {};

  function format(cultureOrFormat) {
    let culture = neutralCulture;
    let format = cultureOrFormat;
    ...
    return format.replace(_formatPlaceHolderRE,
      function(str, match) {
        ...
        const formatter = _formatters[typeName(value)];
        if (formatter) {
          let formatSpec = '';
          const formatIndex = match.indexOf(':');
          if (formatIndex > 0) {
  ...
  function trim(s, tc) {
    if (tc || !String.prototype.trim) {
      tc = tc ? tc.join('') : null;
      const r = tc ? new RegExp('^[' + tc + ']+|[' + tc + ']+$', 'g') : /^\s+|\s+$/g;
      return s.replace(r, '');
    }
    return s.trim();
  }
  function trimStart(s, tc) {
    const r = tc ? new RegExp('^[' + tc.join('') + ']+') : /^\s+/;
    return s.replace(r, '');
  }
  ...
  function bind(fn, o) {
    ...
    const cache = name ? o.$$b || (o.$$b = {}) : null;
    let binding = cache ? cache[name] : null;
  ...
  function bindAdd(binding, value) {
    ...
    const fnList = [].concat(binding._fnList || binding, value);
  ...
  function bindSub(binding, value) {
    ...
    let fnList = binding._fnList || [binding];
    const index = fnList.indexOf(value);
    if (index >= 0) {
      if (fnList.length == 1) {
        return null;
      }
  ...
  function _captureObservers(observers) {
    const registeredObservers = _observerStack;
    const observerCount = registeredObservers.length;

    if (observerCount) {
      observers = observers || [];
      for (let i = 0; i < observerCount; i++) {
        const observer = registeredObservers[i];
  ...
  function _updateTask(task, result, error) {
    if (task.status == 'pending') {
      ...
      const continuations = task._continuations;
      task._continuations = null;

      let i = 0;
      const c = continuations.length;
      for (; i < c; i++) {
  ...
  const currentCulture = {name: 'en-us', dtf: neutralCulture.dtf, nf: neutralCulture.nf};

  // Formatting Helpers

  function _commaFormatNumber(number, groups, decimal, comma) {
    let decimalPart = null;
    const decimalIndex = number.indexOf(decimal);
    if (decimalIndex > 0) {
    ...
    let groupIndex = 0;
    let groupSize = groups[groupIndex];
    ...
    while (!done) {
      ...
      const part = number.substr(startIndex, length);
      if (s.length) {
        s = part + comma + s;
      }
      else {
  ...
  _formatters['Date'] = function(dt, format, culture) {
    ...
    while (true) {
      const index = _dateFormatRE.lastIndex;
      const match = _dateFormatRE.exec(format);
  ...
  const _classMarker = 'class';
  const _interfaceMarker = 'interface';
  ...
  function createType(typeName, typeInfo, typeRegistry) {
    ...
    if (Array.isArray(typeInfo)) {
      ...
      if (typeInfo.length >= 2) {
        ...
        if (!type){
          return console.warn({typeName,typeInfo,typeRegistry});
        }
  ...
  function type(s) {
    const nsIndex = s.indexOf('.');
    const ns = nsIndex > 0 ? _modules[s.substr(0, nsIndex)] : global;
    const name = nsIndex > 0 ? s.substr(nsIndex + 1) : s;
  ...
  function canAssign(type, otherType) {
    ...
    if (type.$type == _classMarker) {
      ...
      while (baseType) {
        const interfaces = baseType.$interfaces;
  ...
  function module(name, implementation, exports) {
    const registry = _modules[name] = {$name: name};
